<template>
  <view class="container">
    ...
    >
      ...
      <!-- 订单卡片 -->
      <view v-for="order in filteredOrders" :key="order.id" v-if="order.inOut=== 0">
  ...
  methods: {
	...
	gotoOrderDetail(id) {
	  ...
	  if (order) {
	    uni.navigateTo({
			// console.log
	      url: `/pages/in_order_detail/in_order_detail?orderId=${encodeURIComponent(JSON.stringify(order.id))}`
